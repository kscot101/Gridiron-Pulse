...
Completed-season inputs may be cached; current Worker, roster, schedule and QB
depth inputs are freshly requested. Failed runs do not replace the public feed.
"""
import argparse
import copy
import json
...
TD_ALIASES = {
    'passingTouchdowns':['passingTD','passingTds','passingTDs','passing_tds'],
    'rushingTouchdowns':['rushingTD','rushingTds','rushingTDs','rushing_tds'],
    'receivingTouchdowns':['receivingTD','receivingTds','receivingTDs','receiving_tds'],
}
...
def stat_total(p, keys):
    ...
    for key in keys:
        ...
        if value is None:
            for alias in TD_ALIASES.get(key,[]):
                value = number(stats.get(alias))
...
def main():
    ...
    import v21_player_context_runner
    ...
    if stated_season is not None and int(stated_season)!=args.season:
        raise ValueError('Season Worker season does not match forecast')
    ...
    if int(source['season'].max())!=args.season-1:
        raise ValueError('Historical source is missing the last completed season')
    ...
    if not schedule:
        raise ValueError('No verifiable upcoming regular-season schedule')

    qb_teams = sorted({team(p.get('team')) for p in players if str(p.get('positionGroup') or p.get('position')).upper()=='QB'})
    def get_depth(tm):
        try:
            code = {'WAS':'wsh'}.get(tm,tm.lower())
            payload = request_json('https://site.api.espn.com/apis/site/v2/sports/football/nfl/teams/'+code+'/depthcharts')
            require_fresh(payload.get('timestamp'),'QB depth '+tm)
            if int((payload.get('season') or {}).get('year',0))!=args.season:
                raise ValueError('QB depth season mismatch')
            if team((payload.get('team') or {}).get('abbreviation'))!=tm:
                raise ValueError('QB depth team mismatch')
    ...
    for row in rows:
        ...
        played,remaining = number(p.get('actualGames')),number(p.get('teamRemainingGames'))
        ...
        if not history.empty:
            ...
            reported,context = number(row.get('last_year_reported_rate')),number(row.get('last_year_context_rate'))
            ...
            actual,rem = stat_total(p,keys),number(row.get('candidateRemainingGames'))
        ...
        if row.get('quarterbackIdentityStatus')=='unknown':
            assert row.get('newQuarterback') is None or pos=='QB'
            assert row.get('quarterbackFactor')==1.0
        if row.get('priorQuarterbackId') and row.get('priorQuarterbackId')==row.get('targetQuarterbackId'):
            assert row.get('newQuarterback') is not True
        for key in ('candidateProjection','candidateTouchdowns','nextGameRate','nextGameTouchdownRate'):
            ...
            row[key] = value
    policy = json.loads((args.research/'model-lab/results/v21-player-context-latest/policy.json').read_text())
    ...
    examples = [{k:p.get(k) for k in ('playerName','team','targetQuarterback','priorQuarterbackId','targetQuarterbackId','newQuarterback','quarterbackFactor','nextOpponent','nextGameId','nextGameRate')} for p in rows if p.get('playerName') in ('CeeDee Lamb','Puka Nacua','Amon-Ra St. Brown')]
    report = {'generatedAt':generated,'sourceGeneratedAt':source_stamp,'workflowRun':os.environ.get('GITHUB_RUN_ID'),'implementationCommit':os.environ.get('GITHUB_SHA'),'players':len(rows),'contextEligiblePlayers':sum(bool(p.get('context_eligible')) for p in rows),'unresolvedQuarterbackTeams':unresolved,'quarterbackDepthErrors':{tm:error for tm,value,error in depth_results if error},'quarterbacks':qb_map,'identityExamples':examples,'missingYardageProjections':sum(p.get('candidateProjection') is None for p in rows),'missingTouchdownProjections':sum(p.get('candidateTouchdowns') is None for p in rows),'historicalBacktestRerun':False,'formulaCoefficientsChanged':False}
